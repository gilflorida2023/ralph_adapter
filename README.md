...
### `profiler/` — Meta-Ralph: auto-discovers model configs

Uses Ralph's own loop pattern to test models and generate configs.
...
The profiler:
1. Sends 5 standard test prompts to the model
2. Analyzes each raw response for JSON structure, quote wrapping, top-level keys, etc.
3. Detects which normalizers are needed
4. Writes `models/<model>.yaml`
5. Validates the config passes all tests
